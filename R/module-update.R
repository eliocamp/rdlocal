...
i18n_module_update <- function(module_path, package_path, dry_run = FALSE) {
  ...
  if (!dir.exists(old_tdir)) {
    stop(module_path, " does not look like a translation module (no inst/translations/)")
  }
  ...
  regen_note <- NULL
  withCallingHandlers(
    i18n_module_create(package_path, language = "und",
                       module_name = paste0(package, ".und"),
                       module_path = tmp_module, rstudio_project = FALSE),
    warning = function(w) { regen_note <<- conditionMessage(w); invokeRestart("muffleWarning") })
  ...
  for (f in intersect(new_files, old_files)) {
    c1 <- new.env(); c1$unchanged <- 0L; c1$fuzzy <- 0L; c1$new <- 0L; c1$dropped <- 0L
    merged[[f]] <- merge_topic(yaml::read_yaml(file.path(old_tdir, f)),
                               yaml::read_yaml(file.path(new_tdir, f)), c1)
    for (k in names(total)) total[[k]] <- total[[k]] + c1[[k]]
    if (c1$fuzzy) fuzzy_topics <- c(fuzzy_topics, f)
  ...
  for (f in setdiff(old_files, new_files)) {
    total$dropped  <- total$dropped + count_leaves_topic(yaml::read_yaml(file.path(old_tdir, f)))
    dropped_topics <- c(dropped_topics, f)
  }

  report_update(total, fuzzy_topics, new_topics, dropped_topics, regen_note, dry_run)

  if (!dry_run) {
    mdir <- file.path(module_path, "man_original")
    for (f in names(merged)) yaml::write_yaml(merged[[f]], file.path(old_tdir, f))
    for (f in setdiff(old_files, new_files)) {
...
merge_topic <- function(old, new, counts) {
  ...
  for (sec in new_secs) out[[sec]] <- merge_node(old[[sec]], new[[sec]], counts)
...
merge_node <- function(old, new, counts) {
  if (is.character(new$original)) {
    if (is.null(old) || !is.character(old$original)) {   # brand new (or changed shape)
      counts$new <- counts$new + 1L
      return(new)
    }
    if (identical(old$original, new$original)) {         # unchanged: keep translation
      counts$unchanged <- counts$unchanged + 1L
      old$spans <- new$spans                             # refresh example values
      return(old)
    }
    if (is.null(old$translation)) {                      # changed but never translated
      counts$unchanged <- counts$unchanged + 1L
      return(new)
    }
    ...
    new$fuzzy             <- TRUE
    if (!is.null(old$ifdef)) new$ifdef <- old$ifdef
    return(new)
  }
  out <- list()                                          # itemlist: merge per item name
  for (nm in names(new)) out[[nm]] <- merge_node(old[[nm]], new[[nm]], counts)
...
count_leaves <- function(node) {
  if (!is.list(node)) return(0L)                         # meta (needs_review, untranslatable)
  if (is.character(node$original)) return(1L)
  sum(vapply(node, count_leaves, integer(1)))
}
count_leaves_topic <- function(topic) sum(vapply(topic, count_leaves, integer(1)))

report_update <- function(total, fuzzy_topics, new_topics, dropped_topics, regen_note, dry_run) {
  message(if (dry_run) "i18n_module_update() (dry run) would change:" else "i18n_module_update():")
  message(sprintf("  %d unchanged  |  %d fuzzy  |  %d new  |  %d dropped",
                  total$unchanged, total$fuzzy, total$new, total$dropped))
  listing <- function(label, x) if (length(x)) {
  ...
  if (total$fuzzy) {
    message("  -> review each fuzzy section, then delete its 'fuzzy:' and 'previous_original:' lines")
  }
  if (!is.null(regen_note)) message("  note (from the new source): ", regen_note)
}
...
bump_translates <- function(module_path, package, version) {
  ...
  if (length(i)) lines[i[1]] <- paste0("Translates: ", package, " (== ", version, ")")
